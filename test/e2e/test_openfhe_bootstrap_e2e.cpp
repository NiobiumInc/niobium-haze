...
// phasefs11: minimal fs07-prefix that triggers Cheby failure. Run only the
// pre-CtS haze pipeline + d2h flush + refresh, then Cheby. If this passes,
// the failure requires CtS (or downstream) on haze. If this fails, just the
// pre-CtS work + d2h is enough.
TEST_CASE("phasefs11 fs07-pre-CtS-only + Cheby",
          "[.nio][.fsladder][e2e]") {
    using namespace lbcrypto;
    namespace ops = haze::test::ops;
    REQUIRE(hazeDeviceReset() == HAZE_SUCCESS);
    ...
    // OpenFHE pre-CtS + CtS + Conj + Add + Rescale (mirrors fs07 OpenFHE side).
    auto raised = ct->Clone();
    while (raised->GetNoiseScaleDeg() > 1)
        algo->ModReduceInternalInPlace(raised, compositeDegree);
    {
    ...
    // HAZE side: ONLY do pre-CtS work (no CtS, no conj, etc.). Then d2h-flush.
    auto haze_ct = ops::h2d_ct(n.ctx, ct);
    auto adjusted = [&]() {
        auto r = ops::clone_ct(n.ctx, haze_ct);
        while (r.noise_scale_deg() > 1) r = ops::rescale(n.ctx, std::move(r));
    ...
    hr = ops::eval_mult_scalar_for_test(n.ctx, hr, pre/(static_cast<double>(K_UNIFORM) * N));
    hr = ops::rescale(n.ctx, hr);
    (void)ops::d2h_ct(n.ctx, hr);
    hr = ops::h2d_ct(n.ctx, raised);  // refresh
    auto h_ctxtEnc_haze = ops::eval_coeffs_to_slots(n.ctx, bk, bk.cts_matrices_fft, hr);
    (void)ops::d2h_ct(n.ctx, h_ctxtEnc_haze);
    // ADD: ops::conjugate
    auto h_pre_cts = ops::h2d_ct(n.ctx, ctxtEnc);
    auto h_conj = ops::conjugate(n.ctx, h_pre_cts, bk.conjugation_key);
    (void)ops::d2h_ct(n.ctx, h_conj);
    // ADD: ops::sub (mimics fs07 step C1)
    h_pre_cts = ops::h2d_ct(n.ctx, ctxtEnc);
    h_conj = ops::h2d_ct(n.ctx, ctxtEnc);  // dummy conj
    auto h_ei = ops::sub(n.ctx, h_pre_cts, h_conj);
    (void)ops::d2h_ct(n.ctx, h_ei);
    // ADD: ops::add (mimics fs07 step C2)
    h_pre_cts = ops::h2d_ct(n.ctx, ctxtEnc);
    h_conj = ops::h2d_ct(n.ctx, ctxtEnc);
    h_pre_cts = ops::add(n.ctx, h_pre_cts, h_conj);
    (void)ops::d2h_ct(n.ctx, h_pre_cts);
    // ADD: ops::mult_monomial_for_test (mimics fs07 step D)
    h_ei = ops::h2d_ct(n.ctx, ctxtEnc);  // dummy ei
    h_ei = ops::mult_monomial_for_test(n.ctx, h_ei, 3 * n.slots);
    (void)ops::d2h_ct(n.ctx, h_ei);
    // ADD: ops::rescale (mimics fs07 step E)
    h_pre_cts = ops::h2d_ct(n.ctx, ctxtEnc);
    if (h_pre_cts.noise_scale_deg() == 2) h_pre_cts = ops::rescale(n.ctx, h_pre_cts);
    (void)ops::d2h_ct(n.ctx, h_pre_cts);

    // Now Cheby on a fresh upload of OpenFHE's post-pipeline ctxtEnc.
    auto ref = n.ctx.cc->EvalChebyshevSeries(ctxtEnc, nio_g_coefficientsUniform, -1.0, 1.0);
    auto haze_for_cheby = ops::h2d_ct(n.ctx, ctxtEnc);
    auto out = ops::eval_chebyshev_series_for_test(n.ctx, haze_for_cheby, nio_g_coefficientsUniform);
    REQUIRE(haze_vs_openfhe(n.ctx, out, ref, "fs11") == 0);
}

// phasefs10: do the ENTIRE pre-Cheby pipeline ONLY on OpenFHE (no haze).
// Then upload OpenFHE's resulting ctxtEnc into a fresh haze context and run
// Cheby. If this fails, the bug is data-dependent on ctxtEnc's byte pattern.
// If it passes, then fs07's pre-Cheby haze ops are somehow corrupting state.
TEST_CASE("phasefs10 OpenFHE-only pre-Cheby + haze Cheby",
          "[.nio][.fsladder][e2e]") {
    using namespace lbcrypto;
    namespace ops = haze::test::ops;
    REQUIRE(hazeDeviceReset() == HAZE_SUCCESS);
    ...
    if (ctxtEnc->GetNoiseScaleDeg() == 2)
        algo->ModReduceInternalInPlace(ctxtEnc, compositeDegree);
    std::cerr << "  [fs10] pre-Cheby ctxtEnc level=" << ctxtEnc->GetLevel()
              << " nsd=" << ctxtEnc->GetNoiseScaleDeg()
              << " sf=" << ctxtEnc->GetScalingFactor()
              << " towers=" << ctxtEnc->GetElements()[0].GetNumOfElements() << "\n";

    // Cheby on both sides (haze never saw the pre-Cheby work).
    auto ref = n.ctx.cc->EvalChebyshevSeries(ctxtEnc, nio_g_coefficientsUniform, -1.0, 1.0);
    for (auto &elem : ctxtEnc->GetElements()) elem.SetFormat(Format::EVALUATION);
    auto haze_ct = ops::h2d_ct(n.ctx, ctxtEnc);
    auto out = ops::eval_chebyshev_series_for_test(n.ctx, haze_ct, nio_g_coefficientsUniform);
    std::size_t bad = haze_vs_openfhe(n.ctx, out, ref, "fs10");
    REQUIRE(bad == 0);
}

// phasefs12 — regression test for niobium-fhetch's openfhe_cprobe_save_dcrt_poly
// fix. Without the running_p() guard in niobium-fhetch's save_dcrt_poly probe,
// a MultByMonomialInPlace on the OpenFHE side (even on a discarded clone)
// leaks captured monomial state into the next haze recording, and the haze
// Chebyshev call after it produces 44/44 wrong polys. With the fix, this
// passes 0/44.
TEST_CASE("phasefs12 MultByMonomial-then-haze-Cheby byte parity",
          "[.nio][.fsladder][e2e]") {
    using namespace lbcrypto;
    namespace ops = haze::test::ops;
    REQUIRE(hazeDeviceReset() == HAZE_SUCCESS);
    ...
    if (ctxtEnc->GetNoiseScaleDeg() == 2)
        algo->ModReduceInternalInPlace(ctxtEnc, compositeDegree);
    // KEY STEP: fire MultByMonomial outside any haze recording. The probe's
    // running_p() guard must prevent state leakage into the next recording.
    {
        auto throwaway = ctxtEnc->Clone();
        algo->MultByMonomialInPlace(throwaway, 3 * n.slots);
    }
    for (auto &elem : ctxtEnc->GetElements()) elem.SetFormat(Format::EVALUATION);
...
// phasefs15: TRUE single-trace bootstrap. Calls ops::* helpers in
// fs07's exact sequence, but inline — NO intermediate d2h_ct, NO
// refresh, ONE final d2h at the very end. Mirrors what ops::bootstrap
// emits, but written out long-form so we can adjust the sequence
// incrementally to find the divergence root cause.
TEST_CASE("phasefs15 single-trace bootstrap, ONE final d2h",
          "[.nio][.fsladder][e2e]") {
    using namespace lbcrypto;
    namespace ops = haze::test::ops;
    REQUIRE(hazeDeviceReset() == HAZE_SUCCESS);
    ...
    // Compute ALL OpenFHE references BEFORE any haze compute, so OpenFHE
    // CPROBES never fire while recording is active.
    auto algo = n.ctx.cc->GetScheme();
    auto fhe_ckks = std::dynamic_pointer_cast<FHECKKSRNS>(algo->GetFHE());
    const auto &precom = *fhe_ckks->GetBootPrecomMap().at(n.slots);
    const std::uint32_t compositeDegree = cp->GetCompositeDegree();
    // OpenFHE references — all computed BEFORE haze starts so CPROBES
    // are skipped (running_p() == false).
    auto raised_ref = ct->Clone();
    while (raised_ref->GetNoiseScaleDeg() > 1)
        algo->ModReduceInternalInPlace(raised_ref, compositeDegree);
    {
        const double targetSF = cp->GetScalingFactorReal(0);
        const double sourceSF = raised_ref->GetScalingFactor();
        const std::uint32_t numTowers = raised_ref->GetElements()[0].GetNumOfElements();
        ...
        const double adjustmentFactor = (targetSF/sourceSF) * (modToDrop/sourceSF)
                                        * std::pow(2.0, -correction);
        n.ctx.cc->EvalMultInPlace(raised_ref, adjustmentFactor);
        algo->ModReduceInternalInPlace(raised_ref, compositeDegree);
        raised_ref->SetScalingFactor(targetSF);
    }
    {
    ...
    n.ctx.cc->EvalMultInPlace(raised_ref, pre / (static_cast<double>(K_UNIFORM) * N));
    algo->ModReduceInternalInPlace(raised_ref, compositeDegree);
    auto ctxtEnc_ref = fhe_ckks->EvalCoeffsToSlots(precom.m_U0hatTPreFFT, raised_ref);
    const auto &evalKeyMap = n.ctx.cc->GetEvalAutomorphismKeyMap(ctxtEnc_ref->GetKeyTag());
    auto conj_ref = FHECKKSRNS::Conjugate(ctxtEnc_ref, evalKeyMap);
    auto ctxtEncI_ref = n.ctx.cc->EvalSub(ctxtEnc_ref, conj_ref);
    n.ctx.cc->EvalAddInPlace(ctxtEnc_ref, conj_ref);
    algo->MultByMonomialInPlace(ctxtEncI_ref, 3 * n.slots);
    if (ctxtEnc_ref->GetNoiseScaleDeg() == 2) {
        algo->ModReduceInternalInPlace(ctxtEnc_ref, compositeDegree);
        algo->ModReduceInternalInPlace(ctxtEncI_ref, compositeDegree);
    }
    auto cheby_R = algo->EvalChebyshevSeries(ctxtEnc_ref, nio_g_coefficientsUniform, -1.0, 1.0);
    auto cheby_I = algo->EvalChebyshevSeries(ctxtEncI_ref, nio_g_coefficientsUniform, -1.0, 1.0);

    // Now: haze pipeline. NO OpenFHE cc-> calls during this section.
    auto haze_ct = ops::h2d_ct(n.ctx, ct);
    auto adjusted = [&]() {
        auto r = ops::clone_ct(n.ctx, haze_ct);
        while (r.noise_scale_deg() > 1) r = ops::rescale(n.ctx, std::move(r));
    ...
    if (scalar != 1) h_ctxtEnc = ops::mult_int_scalar_for_test(n.ctx, h_ctxtEnc, scalar);
    h_ctxtEnc = ops::rescale(n.ctx, h_ctxtEnc);
    auto h_ctxtDec = ops::eval_slots_to_coeffs(n.ctx, bk, bk.stc_matrices_fft, h_ctxtEnc);
    const std::uint64_t corFactor = static_cast<std::uint64_t>(1)
        << static_cast<std::uint64_t>(correction);
    ...
    // ONE d2h here. Everything before this is one recording.
    REQUIRE(haze_vs_openfhe(n.ctx, h_ctxtDec, ref_full, "fs15") == 0);
}
...
TEST_CASE("phasefs05 niobium-config {4,4} full-slot ops::bootstrap byte-equal e2e",
    ...
    std::cerr << "  [phasefs05] bad_towers=" << bad << " of "
              << (2 * haze_refreshed.towers()) << "\n";
    // Sample a few values to see if outputs are zero / random / shifted.
    for (std::size_t t : {std::size_t{0}, std::size_t{5}, std::size_t{11}}) {
        if (t >= haze_refreshed.towers()) continue;
        const auto &rv = ct_ref->GetElements()[0].GetElementAtIndex(static_cast<usint>(t)).GetValues();
        std::cerr << "  [phasefs05.dbg] t=" << t << " ref={"
                  << rv[0].template ConvertToInt<std::uint64_t>() << ","
                  << rv[1].template ConvertToInt<std::uint64_t>() << ","
                  << rv[2].template ConvertToInt<std::uint64_t>() << "} haze={"
