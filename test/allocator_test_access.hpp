...
class AllocatorTestAccess {
  public:
    ...
    // Number of live DevAddrs (alloc_set_ membership); lets a test assert that a
    // failed allocation leaves nothing allocated.
    static std::size_t alloc_set_size(const DeviceAllocator &a) noexcept {
        HazeLockGuard lock(a.mutex_);
        return a.alloc_set_.size();
    }

    // Push a DevAddr to the FRONT of the free list. allocate pops from the back,
    // so [wedge, recyclable] makes a two-poly batch recycle `recyclable` first,
    // then pop `wedge`; passing a still-LIVE addr as `wedge` sends that second pop
    // down the "recycled addr already live" PoolMapDesync path — the only way to
    // exercise allocate_many's mid-batch rollback (unreachable via the public
    // API), and it depends on the allocator's LIFO free-list discipline.
