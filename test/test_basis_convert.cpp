...
// this file.
static constexpr size_t kSrcLimbs = 12;
static constexpr size_t kPLimbs = 4;
static constexpr uint64_t kBigBase[kSrcLimbs + kPLimbs] = {
...
static void configure_sixteen_moduli() {
    REQUIRE(hazeDeviceReset() == HAZE_SUCCESS);
    REQUIRE(hazeSetRingDimension(kRingDim) == HAZE_SUCCESS);
    for (int i = 0; i < static_cast<int>(kSrcLimbs + kPLimbs); ++i) {
...
static std::vector<uint64_t> make_residue(uint64_t prime, uint64_t seed) {
...
fast_base_convert(const std::vector<std::vector<uint64_t>> &residues,
    ...
    for (uint64_t s = 0; s < kRingDim; ++s) {
        ...
        for (size_t j = 0; j < dst_base.size(); ++j) {
            ...
            auto src_it = std::find(src_base.begin(), src_base.end(), p);
...
static std::vector<std::vector<uint64_t>>
rescale_fbc(const std::vector<std::vector<uint64_t>> &residues,
            const std::vector<uint64_t> &src_base, const std::vector<uint64_t> &rescale_base) {
    ...
    for (size_t i = 0; i < src_base.size(); ++i) {
        const bool in_rescale =
            std::find(rescale_base.begin(), rescale_base.end(), src_base[i]) != rescale_base.end();
        if (!in_rescale) {
    ...
    for (uint64_t q : rescale_base) {
        auto it = std::find(src_base.begin(), src_base.end(), q);
...
// variant explicitly.
static std::vector<std::vector<std::vector<uint64_t>>> dig_decomp(
    ...
    for (size_t d = 0; d < digit_bases.size(); ++d) {
        ...
        for (size_t i = 0; i < digit_bases[d].size(); ++i) {
            auto it = std::find(src_base.begin(), src_base.end(), digit_bases[d][i]);
...
static std::vector<void *> allocate_and_h2d(const std::vector<std::vector<uint64_t>> &residues) {
...
static std::vector<void *> allocate_dst(size_t count) {
...
static void free_all(const std::vector<void *> &ptrs) {
    for (void *p : ptrs) {
...
static void check_against_reference(const std::vector<void *> &dst,
